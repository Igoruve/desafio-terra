...
import { issueTypeNotProvided, 
    deviceNotProvided, browserNotProvided, 
    clientCommentNotProvided, pageUrlNotProvided } from "../../utils/errors/issueErrors.js";

import { customAlphabet } from "nanoid";

const getRandomCode = customAlphabet("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", 6);
async function getAllIssues() {
    const issues = await issueModel.find().populate("client");
    return issues;
}
...
async function getIssuesByStatus(status){
    const issues = await issueModel.find({status:status}).populate("client");
    return issues;
}
...
async function getIssuesByDevice(device){
    const issues = await issueModel.find({device:device}).populate("client");
    return issues;
}
